...
export const PERSONIO_CAPABILITIES: PersonioCapability[] = [
  ...
  {
    id: 'personio_create_person',
    title: 'Create Employee (Personio, HR)',
    description: 'Onboard a new employee: person + employment (legal entity, start date, position, hours, supervisor). Company-scoped instances force their own legal entity.',
    risk: 'write',
    examples: [{ firstName: 'Ny', lastName: 'Medarbejder', email: 'nm@example.com', employmentStartDate: '2026-09-01', position: 'Consultant' }],
    identifierFormats: ['legalEntityId (ignored on scoped instances)'],
    safetyNotes: ['Requires PERSONIO_ENABLE_WRITES=true. Email is immutable once set.'],
    keywords: ['opret medarbejder', 'onboard', 'ny ansat', 'create employee', 'hire'],
  },
  {
    id: 'personio_update_employment',
    title: 'Update Employment (Personio, HR)',
    description: 'Change position, supervisor, weekly hours, cost center, office, org unit, dates — and offboard via employment_end_date/termination. Company transfers only on the all-companies instance.',
    risk: 'write',
    examples: [{ personId: '42', patch: { position: 'Senior Consultant' } }, { personId: '42', patch: { employment_end_date: '2026-12-31' } }],
    identifierFormats: ['patch uses Personio employment field names'],
    safetyNotes: ['Requires PERSONIO_ENABLE_WRITES=true.'],
    keywords: ['ansættelse', 'forfremmelse', 'stilling', 'chef', 'timer', 'fratrædelse', 'offboard', 'terminate', 'promotion', 'transfer'],
  },
  {
    id: 'personio_create_compensation',
    title: 'Create Compensation (Personio, HR)',
    description: 'Add salary, hourly rate, one-time or recurring compensation. Cannot be edited/deleted afterwards via API.',
    risk: 'write',
    examples: [{ personId: '42', type: 'FIXED_SALARY', amount: 45000, interval: 'MONTHLY', effectiveDate: '2026-09-01' }],
    identifierFormats: ['type: FIXED_SALARY | HOURLY_SALARY | ONE_TIME | RECURRING'],
    safetyNotes: ['Requires PERSONIO_ENABLE_WRITES=true. IRREVERSIBLE — no edit/delete API for compensation.'],
    keywords: ['løn', 'lønstigning', 'bonus', 'raise', 'salary', 'compensation', 'timeløn'],
  },
  {
    id: 'personio_manage_document',
    title: 'Update/Delete Document (Personio, HR)',
    description: 'Update a document’s metadata or delete it (permanent). Uploading is personio_upload_document.',
    risk: 'write',
    examples: [{ action: 'delete', documentId: 'abc' }],
    identifierFormats: ['action: update | delete'],
    safetyNotes: ['Requires PERSONIO_ENABLE_WRITES=true. Delete is permanent.'],
    keywords: ['dokument', 'slet', 'ret dokument', 'document', 'delete'],
  },
  {
